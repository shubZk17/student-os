...
const STAGES: { key: ApplicationStage; label: string; color: string }[] = [
  ...
  { key: 'INTERVIEW', label: 'Interview', color: 'border-purple-500/40' },
              ...
              <div className="space-y-3 overflow-y-auto flex-1 pr-1">
                {colApps.length === 0 ? (
                  ...
                  colApps.map((app) => (
                    <div
                      key={app.id}
                      className="p-3.5 rounded-2xl bg-slate-950 border border-slate-800/90 hover:border-indigo-500/40 transition shadow-sm space-y-2.5"
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-center space-x-2.5">
                          ...
                          <div>
                            ...
                            <span className="text-[10px] text-slate-400">{app.company_name}</span>
                          </div>
                        </div>
                      ...
                      {/* Interview Date Pill */}
                      {app.interview_date && (
                        <div className="flex items-center space-x-1 text-[10px] text-purple-400 bg-purple-950/40 border border-purple-500/20 px-2 py-1 rounded-lg">
                      ...
                      {app.notes && (
                        <p
                          ...
                          className="text-[11px] text-slate-400 bg-slate-900/60 p-2 rounded-xl border border-slate-800 cursor-pointer hover:border-slate-700 transition line-clamp-2"
                        >
                          {app.notes}
                        </p>
                      )}
                      ...
                      <div className="pt-2 border-t border-slate-900 flex items-center justify-between text-[10px]">
                        ...
                        <select
                          ...
                          className="bg-slate-900 border border-slate-800 rounded-lg px-2 py-0.5 text-[10px] text-slate-300 focus:outline-none"
                        >
                          {STAGES.map((s) => (
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            ...
            <textarea
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-3 text-xs text-slate-200 focus:outline-none focus:border-indigo-500"
            />
