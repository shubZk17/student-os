...
func main() {
	...
	if err := db.Pool.QueryRow(ctx, `SELECT COALESCE(array_agg(name), '{}') FROM skills`).Scan(&known); err != nil {
		log.Fatalf("[FATAL] Load skills: %v", err)
	}
	...
	log.Printf("[INFO] Ingestion finished: %d opportunities, %d/%d boards failed", total, failed, len(boards))
	if failed == len(boards) && len(boards) > 0 {
